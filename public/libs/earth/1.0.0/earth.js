...
(function () {
    ...
    function interpolateField(globe, grids) {
        ...
        //var ctxbg = canvasbg.getContext('2d');
        //var img = new Image;
        //img.src = '../public/data/ice802645.jpg';//src也可以是从文件选择控件中取得。
        // var data;
        //img.onload = function(){
        //    ctxbg.drawImage(img, 0, 0);
        //    data = ctxbg.getImageData(0, 0, img.width, img.height).data;//读取整张图片的像素。
        ...
        function interpolateColumn(x) {
            ...
            //databg = ctxbg.getImageData(0, 0, imgbg.width, imgbg.height).data;//读取整张图片的像素。
            //mask.imageData.data=databg;
            //mask.imageData=ctxbg.getImageData;
        }
    ...
    var backgroudValues = new Array();

    //dataTable[i].push(lineData[i])
    $.ajax({
        async: false,
        type: "GET",
        ...
        success: function (content) {
            ...
            for (var dimIndex = 0; dimIndex < 5; dimIndex++) {
                backgroudValues[dimIndex] = new Array();
            }
            for (var lineIndex in data) {
                var dimValue = data[lineIndex].split(" ");

                backgroudValues[0].push(dimValue[0]);
                backgroudValues[1].push(dimValue[1]);
                backgroudValues[2].push(dimValue[2]);
                backgroudValues[3].push(dimValue[3]);
                backgroudValues[4].push(dimValue[4]);
    ...
    $.ajax({
        ...
        success: function (content) {
            ...
            var lineValue = data[0].split(";");
            imageWidth = lineValue.length-1;


            var data = content.split('\n');
            for (var longIndex = 0; longIndex < imageWidth; longIndex++) {
                layerValues[longIndex] = new Array();
                for (var latiIndex = 0; latiIndex < imageHeight; latiIndex++) {
                    layerValues[longIndex][latiIndex] = new Array();
                }
            }

            var latiIndex2 = 0;
            for (var lineIndex in data) {
                var lineValue = data[lineIndex].split(";");
                for (var longIndex2 = 0; longIndex2 < imageWidth; longIndex2++) {
                    if (lineValue.length > 1) {
                        var pixelValue = lineValue[longIndex2].split(" ");
                        var r = pixelValue[0];
                        var g = pixelValue[1];
                        var b = pixelValue[2];
                        //for (var latiIndex2 = 0; latiIndex2 < 4; latiIndex2++) {
                        layerValues[longIndex2][latiIndex2].push(r);
                        layerValues[longIndex2][latiIndex2].push(g);
                        layerValues[longIndex2][latiIndex2].push(b);
                        //}
                    }
    ...
    function getColorByCoordinate(x, y) {

        var bgColor = new Array();
        var Δx = 360/imageWidth;
        var Δy = 180/imageHeight;
        var x0 = 180;
        var y0 = 90;
        //var j = µ.floorMod(x - x0, 360) / Δx;  // calculate longitude index in wrapped range [0, 360)
        var j = ((x0 - x)) / Δx;
        j = imageWidth - j;
        var i = (y0 - y) / Δy;                 // calculate latitude index in direction +90 to -90
        //i = µ.floorMod(y - y0, 180) / Δy;
    ...
    //function getColorByCoordinate(x, y) {
    //    var bgColor = new Array();
    //    var intX = Math.ceil(x);
    //    var intY = Math.ceil(y);
    //    for (var i = 0; i < backgroudValues[0].length / 1000; i++) {
    //        if (intX == backgroudValues[0][i] && intY == backgroudValues[1][i]) {
    //            bgColor[0] = backgroudValues[2][i];
    //            bgColor[1] = backgroudValues[3][i];
    //            bgColor[2] = backgroudValues[4][i];
    //            bgColor[3] = 128;
    //        }
    //        else {
    //            //bgColor[0]=255;
    ...
    //$.get('data/Ice360180CoorRGB.txt').success(function (content) {
    //    // content就为文件data.txt的文本内容了,注意txt文件的编码需要与html文件的编码一致，最好保存成utf-8
    //    //document.getElementById('boldStuff').innerText=content;
    //    //alert($("#boldStuff")[0].innerText);
    //    var data = content.split('\n');
    ...
    //            lineData = result.split(/\s+/g);//10列
    //            if (lineData.length != colNumber) {
    ...
    //    }
    //    //alert(readData);
    //    var s = dataTable[2][22];
    //    //alert(s);
    //    //alert(dataTable[0].length);
    //
    //});
    ...
    function drawOverlay(field, overlayType) {
        ...
        if (overlayType) {
            if (overlayType !== "off") {
                ...
                //ctx.putImageData(ctxbg.getImageData, 0, 0);
                //imgbg.src = '../public/data/ice802645.jpg';//src也可以是从文件选择控件中取得。
            ...
            //img.onload = function(){
            //    ctxbg.drawImage(img, 0, 0);
            //    var data = ctxbg.getImageData(0, 0, img.width, img.height).data;//读取整张图片的像素。
            //    //console.log(data, data.toString());
            //
            //    for(var i =0,len = data.length; i<len;i+=4){
            //        var red = data[i],//红色色深
